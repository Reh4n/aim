...
const uploaderAPI = (fileData, type) => new Bluebird(async (resolve, reject) => {
    const postFile = async (fileData, type) => {
        ...
        if (type === "telegraph") {
            ...
            form.append("file", createReadStream(filePath));
            const { data } = await axios.post("https://ichikaa.xyz/upload", form, {
                responseType: "json", headers: { ...form.getHeaders() }
            });
            return { host: "ichika", data: { url: data.result.url, name: data.result.originalname, size: data.result.size } };
        }
    }
    try {
